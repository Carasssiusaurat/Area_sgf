...
const Card = ({name, img_url, url, service_id, navigation}) => {
    ...
    return (
      ...
      <View style = {{width : '80%', height : 200, borderRadius : 30, backgroundColor : 'rgba(0, 0, 0, 0.74)', marginTop : 50, marginLeft: 'auto', marginRight : 'auto', flex : 2}}>  
        ...
        <TouchableOpacity style = {{width : '50%', height : '30%', backgroundColor : 'grey', borderRadius : 10, marginLeft : 150, marginTop : 20}} onPress={() => navigation.navigate("Web", {url : url, service_id : service_id})}>
    ...
    return (
      <>
        {response.map((service, index) => (
          <Card key={index} name={service.name} img_url={service.img_url} url={service.connection_url} service_id={service._id} navigation = {navigation}/>
        ))}
      </>
    );
  };
...
export class User_home extends React.Component{
    ...
    get_page = () => {
        ...
        <View style={{width : '100%' ,height : 110, flexDirection : 'row', backgroundColor : 'black', borderRadius : 0, marginLeft : 'auto', marginRight : 'auto'}}>
                ...
                <Text style = {{fontWeight: 'bold' , marginTop : 60, marginLeft : 20 ,resizeMode: 'contain',height: 50, fontSize : 19, color : 'white'}} >Workspace</Text>
          </View>
        );
    }
